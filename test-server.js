...
*/
    ...
    if ( req.method === 'POST' ) {
        ...
        req.on('error', function (err) {
            console.log('POST request: Error when transferring a user profile: ' + err.message);
            res.writeHead(404);
            res.end();
            return;
        });
...
//console.log( userProfileObject.openingHours[0].hours[0] )
                ...
                console.log('POST request: A user profile is empty');
                res.writeHead(404);
                res.end('POST request: A user profile is empty');
            }
        });
